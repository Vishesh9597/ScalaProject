package ca.mcit.bigdata.vishesh
  ...
  class MapJoin[X,Y] (val joinfunc:


                      (X ) => String)(val joinCondition1: (Y) => String)extends Stm_Join[X,Y,JoinValue]{
    override def join(n: List[X], m: List[Y]): List[JoinValue] ={
      val l:Map[String,Y] = m.map(m=>joinCondition1(
        m) ->m).toMap
      n.filter(n=>l.contains(joinfunc(n)))
        .map(n=> JoinValue(n,Some(l(joinfunc(n)))))
